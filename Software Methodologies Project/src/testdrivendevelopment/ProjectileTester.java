...
public class ProjectileTester {
	...
	public void movement(){
		for(float y=1 ; y<100 ; y++){
			...
			//for positive speed
			Projectile bullet1 = new Projectile(null,100,100,y,0,0);
			...
			//for negative speed
			Projectile bullet2 = new Projectile(null,100,100,-y,0,0);
			for(float x=0 ; x<100 ; x+=y){
	...
	public void HitBox(){
		for(int x=0 ; x<200 ; x++){
			for(int y=0 ; y<200 ; y++){
				Projectile bullet = new Projectile(null,x,y,0,0,0);
				/*
				 ...
				 * It also should hold the size of 6 x 12 pixels;
				 */
				Rectangle hitBox = new Rectangle(x+4,y+4,6,12);
				assertEquals(hitBox,bullet.getBounds());
			}
		}
	}
	...
	public void hitBoxPosition(){
		for(float y=1 ; y<100 ; y++){ //assign different speed for the projectile
			...
			for(float x=0 ; x<100 ; x+=y){
				...
				Rectangle hitBox = new Rectangle(100+4,100+(int)x+4,6,12); //create a new rectangle on the expected position
			...
			for(float x=0 ; x<100 ; x+=y){
				...
				Rectangle hitBox2 = new Rectangle(100+4,100-(int)x+4,6,12); //create a new rectangle on the expected position
